...
export const MainMenu = (presence) => {
  ...
  const renderPatientCard = (patient, index) => {
    return (
      <div
        ...
        className={`patientcard ${index > 0 ? "In-progress" : ""}`}
      >
        <div
          className="name"
          key={`card${index}`}
          onClick={() => showPatient(index)}
        >
          {patient.name}
        </div>
        <FontIcon
          ...
          title="Add Feedback"
          iconName="Comment"
          onClick={() => showFeedback(patient.name, index)}
        />
        {index === 0 && (
          <FontIcon
            className="icon"
            ...
            onClick={() => showImageDialog(patient.name, index)}
          />
        )}
      </div>
    );
  ...
  const renderAddFeedback = (index) => {
    return (
      <div key={`addfeedback${index}`} className="addfeedbackcontainer">
        ...
        <DefaultButton
          text="Cancel"
          onClick={() => setIsAddFeedbackVisible(false)}
        />
      </div>
    );
  };
  ...
  const renderImageDialog = (patient) => {
    return (
      <Dialog
        hidden={!isImageDialogVisible}
        dialogContentProps={{
          title: patient.name,
          showCloseButton: false,
          styles: dialogHeaderStyles,
        }}
        modalProps={{
  ...
  //                 clickedKey === 0 &&
  //                 renderAddFeedback(0)}
  //               {isImageDialogVisible && renderImageDialog(people[0])}
  //             </>
  //           )}
  //         </div>
  //         <h3>Next patients</h3>
